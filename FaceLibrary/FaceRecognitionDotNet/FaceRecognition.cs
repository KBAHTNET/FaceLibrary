...
namespace FaceRecognitionDotNet
{
    ...
    public sealed class FaceRecognition : DisposableObject
    {
        ...
        public FaceEncoding FaceEncoding(Image image,
                                               Location knownFaceLocation = null,
                                               int numJitters = 1,
                                               PredictorModel predictorModel = PredictorModel.Small,
                                               Model model = Model.Hog)
        {
            ...
            location.Add(knownFaceLocation);

            var rawLandmarks = this.RawFaceLandmarks(image, location, predictorModel, model);

            var ret = new FaceEncoding(FaceRecognitionModelV1.ComputeFaceDescriptor(this._FaceEncoder, image, rawLandmarks.First(), numJitters));
            rawLandmarks.First().Dispose();
            return ret;
        }
        ...
        public static FaceEncoding LoadFaceEncoding(double[] encoding)
        {
            ...
            var matrix = new Matrix<double>(0, 1);// Matrix<double>.CreateTemplateParameterizeMatrix(0, 1);
            matrix.SetSize(128);
